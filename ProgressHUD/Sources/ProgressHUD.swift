...
public extension ProgressHUD {
    ...
    class var isHudBarTranslucent: Bool {
        get { shared.isHudBarTranslucent }
        set { shared.isHudBarTranslucent = newValue }
    }
    
    class var hudBarStyle: UIBarStyle {
        get { shared.hudBarStyle }
        set { shared.hudBarStyle = newValue }
    }
}
...
public class ProgressHUD: UIView {
    ...
    /// HUD的容器toolbar是否透明
    private var isHudBarTranslucent: Bool = true
    ...
    private var colorBackground = UIColor.clear
    private var colorHUD = UIColor.black.withAlphaComponent(0.5)
    ...
    private func setupToolbar() {
        if toolbarHUD == nil {
            let toolbarHUD = UIToolbar(frame: CGRect.zero)
            toolbarHUD.isTranslucent = isHudBarTranslucent
            toolbarHUD.barStyle = hudBarStyle
